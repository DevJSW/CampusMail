...
public class ViewDirectmailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_view_directmail);
        ...
        mSendDM.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent cardonClick = new Intent(ViewDirectmailActivity.this, SendDM.class);
                cardonClick.putExtra("reciever_id", mPostKey );
                startActivity(cardonClick);
            }
        });

        mPostKey = getIntent().getExtras().getString("heartraise_id");
        mUidKey = getIntent().getExtras().getString("reciever_id");

        mDatabaseUsers = FirebaseDatabase.getInstance().getReference().child("Users");
        mDatabaseUsers.keepSynced(true);

        mDatabaseComments = FirebaseDatabase.getInstance().getReference().child("Directmails");
        mDatabaseComments.keepSynced(true);
        mDatabaseComments.child(mPostKey).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {

                String user_id = (String) dataSnapshot.child("sender_uid").getValue();
